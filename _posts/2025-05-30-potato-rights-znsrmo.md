...
本文对土豆家族中几个关键提权漏洞进行学习分析，注意是学习提权思路，poc具体细节以后有时间再分析。
...
成功劫持 WPAD 之后，会向目标机器会返回一个自定义的 PAC 文件地址，（通常是 [http://attacker_ip/wpad.dat）](http://attacker_ip/wpad.dat%EF%BC%89)，该 PAC 文件中指定浏览器将所有流量代理到攻击者的机器上。文件的示例如下：
...
具体可以参考这里：[http://ohpe.it/juicy-potato/CLSID/](http://ohpe.it/juicy-potato/CLSID/)。
